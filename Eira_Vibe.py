...
def authenticate(username, password):
    ...
    cursor.execute("SELECT id, password_hash, first_name, last_name FROM users WHERE username = ?", (username,))
    ...
    try:
        cursor.execute(
            ...
            (
                ...
                data["medical_aid_number"], data["medical_history"], data["diagnosis"], user_id
            )
        )
        conn.commit()
    ...
    try:
        cursor.execute(
            ...
            (
                data["first_name"], data["last_name"], data["date_of_birth"], data["gender"],
                data["phone"], data["email"], data["address"], data["medical_aid_name"],
                data["medical_aid_number"], data["medical_history"], data["diagnosis"], patient_id
            )
        )
        conn.commit()
...
def get_appointment(appointment_id):
    ...
    appointment = dict(cursor.fetchone()) if cursor.fetchone() else None
    conn.close()
...
def generate_word_report(patient_id, user_id):
    ...
    patient_table = doc.add_table(rows=7, cols=2)
